	...
	<div class="page-data-xiaofei">
		<searchInputItems>
			<searchInputItem name="日期范围">
				<dateEditorDaterange :dateValue.sync="dateRange" :dateClearable='false' @change="searchTable" ></dateEditorDaterange>
			</searchInputItem>
...
var weightPie = null, pricePie = null
	export default {
		...
		mounted() {
	this._ajax({name: 'inmarketdetail/getProdectInmarketdetailTotalPriceAndTotalQuality',param:{customerId:8}})
				.then((function(d) {
					console.log(d);
				}).bind(this))
			this._ajax({name: 'order/getDateOffsetSixToNow',param:{offsetDays:-7}})
						.then((function(d) {
							if (d.aaData !=null) {
								 this.dateRange=d.aaData
							}
						}).bind(this))
