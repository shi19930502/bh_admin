		...
		<elemTable :dataList="dataList" :currentPage='pageNum' :pageSize="pageSize" :pageTotal="pageTotal" :loading="dataLoading" @sizeChange="handleSizeChange" @currentChange="handleCurrentChange" @selectionChange="selectionChange">
		   ...
		   <!-- <el-table-column prop="customerName" label="用户名称">
		    ...
		    <el-table-column prop="supportNum" label="附件数量">
		    	<template slot-scope="scope">
		    		<toolTip :content="scope.row.supportNum">
		...
		methods: {
			...
			send(o){
				...
				var platflow={
					...
					title:o.title,
					comment:o.comment,
					supportNum:o.supportNum,
					noticeStatus:0,
					publishedDate:this._getCurrentDate(),
					userIds:o.customerId,
					userNames:o.customerName
				}
				this.confirm('确定发送？', (function() {
					this._ajax({url:this.tootAPI,name:'platformnoticeflow/create',param:platflow})
					.then((d)=>{
						if(d.state==0){
							this._ajax({url:this.tootAPI,name:'platformnotice/update',param:param})
							.then((f)=>{
								if(f.state==0){
