<template>
	<el-dialog custom-class="jz-modal detection-modal" :title="title" :visible="modal" :before-close="beforeClose" :close-on-click-modal="false" :width="modalWidth">
		...
		<detectionInmarkdetailModal :modal='detectionInmarkdetailModal' v-if='detectionInmarkdetailModal' @close='detectionInmarkdetailModalClose'></detectionInmarkdetailModal>
	...
	export default {
		...
		methods: {
			show(row) {
				...
				this.dataList = [];
				this.form.checkDate = this.getToday
				this.getInitData();
				this.modal = true;
			},
			getInitData() {
				...
				this.$refs['form'].validate((valid) => {
					if(valid) {
						...
						data.forEach((function(o) {
							if(this.form.checkDate && o.checkDate) {
								var time1 = new Date(this.form.checkDate.replace("-", "/").replace("-", "/"));
								var time2 = new Date(o.checkDate.replace("-", "/").replace("-", "/"));
								if(time1 > time2) flag = true
							}
							if(o.sampleNo && o.sampleNo.length > 20) flag1 = true
							if(!o.quarantineId || (o.quarantineId && o.quarantineId.length > 20)) flag2 = true
						}).bind(this))
						if(flag) {
							this.$message({
								type: 'warning',
				...
				this._ajax({
						...
						loading: 'dataLoading'
					 	...
					 	$.each(data, function(i, obj) {
							...
							obj.sellerCode= obj.wholesalerId,
							obj.sellerName= obj.wholesalerName,
							obj.productCode= obj.goodsCode,
							obj.productName= obj.goodsName,
