<template>
	<el-dialog custom-class="jz-modal" :title="modalTitle" :visible="modal" :before-close="beforeClose" :close-on-click-modal="false" :width="modalWidth">
            <el-form class="modal-form" v-if="modal" :model="form" ref="form" :inline="true" :rules="rules" label-width="80px">
                ...
                <el-row>
                    <el-col :xs="24" :sm="24" :md="24" :lg="12" :xl="12">
                         <el-form-item label="渠道" prop="typeCode"  v-if="userModalType === 'add' || userModalType === 'edit'">
                          <selectInput :value.sync="form.typeCode">
                            <el-option v-for="item in  channelList" :key="item.key" :label="item.value" :value="item.key" ></el-option>
                            </selectInput>
                         </el-form-item>
                         <el-form-item label="渠道" prop="typeCode" v-else>
                            <span>{{_dicValue(form.typeCode, channelList)}}</span>
                        </el-form-item>
                    </el-col>
                    <el-col :xs="24" :sm="24" :md="24" :lg="12" :xl="12">
	...
	export default {
		...
		data() {
			...
			return {
                ...
                roleList: [],
                rules: {
                    typeCode: [{ required: true, message: "渠道必选" }],
