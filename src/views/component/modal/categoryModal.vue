<template>
	<el-dialog custom-class="jz-modal" :title="categoryModalType === 'add' ? '新增分类' : '分类编辑'" :visible="modal" :before-close="beforeClose" :close-on-click-modal="false" :width="modalWidth">
            <el-form class="modal-form" v-if="modal" :model="form" ref="form" :inline="true" :rules="rules" label-width="125px">
                <el-row>
                ...
                <el-row>
                    <el-col :xs="24" :sm="24" :md="24" :lg="12" :xl="12">
                        <el-form-item label="父级分类" prop="parentCategoryId" v-if="categoryModalType == 'add'">
                ...
                <el-row>
                    <el-col :xs="24" :sm="24" :md="24" :lg="12" :xl="12">
                        <el-form-item label="市场管理费率" prop="managefee">
                            <inputItem type="managefee" :autosize="true" :value.sync="form.managefee"></inputItem>
                        </el-form-item>
                    </el-col>

                    <el-col :xs="24" :sm="24" :md="24" :lg="12" :xl="12">
                        <el-form-item label="修改子级分类费率" prop="feeEnabled">
                            <switchItem :value.sync="form.feeEnabled"></switchItem>
                        </el-form-item>
                    </el-col>
                </el-row> 
		...
		data() {
			return {
                ...
                rules: {
                    ...
                    vulgo:[{required: true, message: '别名不能为空' }, this._ruleLength(50), this._ruleExist(configs.rootAPI + 'category/listByEntity', '别名')],
                    remark: [this._ruleLength(200)],
                    managefee: [ this._ruleTwoFloat()]
                },
                propss: {
                    label: 'name',
                    value: 'code',
                        ...
                        if(this.categoryModalType !== 'add') {
                            ...
                            .then((function(d) {
                                ...
                                if(this.flag) {
                                    ...
                                    if(o.enabled) {
                                        // 停用,查询子级是否有未停用的
                                        this._ajax({url: this.rootAPI, name: 'category/listByEntity', param: {parentCategoryId: o.code, enabled: 0}})
                                        .then((function(d) {
                                            if(d.aaData.length > 0) {
                                                this.$message({type: 'warning', message: '不能停用,该分类下有子分类未停用'});
                                            }else {
                                        ...
                                        if(o.parentCategoryId.length > 0) {
                                            this._ajax({url: this.rootAPI, name: 'category/listByEntity', param: {code: o.parentCategoryId, enabled: 1}})
                                            .then((function(d) {
                                                if(d.aaData.length > 0) {
