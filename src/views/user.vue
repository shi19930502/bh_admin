  ...
  <div class="page-user">
    ...
    >
      ...
      <el-table-column prop="status" label="用户状态">
        <template slot-scope="scope">
          <tagItem
            ...
            :text="_dicValue(scope.row.status, dicUserState)"
          ></tagItem>
        </template>
      </el-table-column>
      <el-table-column prop="mobile" label="手机号">
        <template slot-scope="scope">
          <inputItem
            :value.sync="scope.row.mobile"
            @blur="changeMobile(scope.row)"
      ...
      <el-table-column prop="typeCode" label="渠道">
        <template slot-scope="scope">
          <toolTip :content="scope.row.typeCode">
            <span>{{_dicValue(scope.row.typeCode, channelList)}}</span>
          </toolTip>
        </template>
      </el-table-column>
        ...
        <template slot-scope="scope">
          <el-button-group>
            ...
            <iconBtn
              ...
              v-if="scope.row.userId != thisUser.userId"
            ></iconBtn>
          </el-button-group>
        </template>
      </el-table-column>
...
export default {
  ...
  mounted() {
    //查询渠道数据
    this._getChannelListOnUse(local.get("sessionUser").typeCode).then(
      function(d) {
        d.aaData.forEach(function(el, i) {
          d.aaData[i].key = el.code;
          d.aaData[i].value = el.name;
        });
        Object.assign(this.channelList, d.aaData);
      }.bind(this)
    );
    console.log(this.rootAPI);
    this._searchDic("USER_STATE").then(
      function(d) {
    ...
    delSubmit(o) {
      this._comfirm("确定删除？")
        ...
        .then(
          function(d) {
            this._ajax({
              url: this.api + "/permission/tsysRoleUser/binDing",
              param: o,
              arr: true
            });
